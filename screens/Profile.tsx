import { Dimensions, Image, Pressable, StyleSheet, Text, View, useColorScheme, FlatList, TextInput, Keyboard, Alert, Animated, BackHandler } from 'react-native'
import React, { useEffect, useState } from 'react'
import {auth, db, storage} from '../lib/firebase-config'
import colors from '../lib/colors'
import { collection, deleteDoc, doc, getDocs, query, where } from 'firebase/firestore'
import { useNavigation } from '@react-navigation/native'
import { sendPasswordResetEmail, signOut, updateCurrentUser, updateEmail, updateProfile } from 'firebase/auth'
...
export default function Profile() {
  ...
  return (
    <View style={[styles.profileScreen, darkMode ? styles.darkProfileScreen : styles.lightProfileScreen]}>
                ...
                <FlatList data={favoriteWallpapers} numColumns={2} 
                    renderItem={({item}) => (
                        <Pressable style={styles.photosWrappers} 
                            ...
                            >
                            <Image source={{uri:item.photoUrl}} style={[styles.wallpapers, selectMode && styles.selectModeWallpapers]}/>
                            {selectMode && 
        ...
        {
        selectMode && 
        <Pressable onPress={() => removeFavoriteWallpapers()} style={({pressed}) => [styles.removeButton, pressed && {backgroundColor:colors.darkOrange}, selectMode && {transform:[{translateY:680}]}, selectedWallpapers.length > 0 && {transform:[{translateY:600}]}]}>
